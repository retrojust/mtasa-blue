...
int CLuaVehicleDefs::CreateVehicle(lua_State* luaVM)
{
    ...
    argStream.ReadBool(bServersideOnly, false);
        ...
        if (pLuaMain)
        {
            ...
            if (pResource)
            {
                // if ( usModel != 570 || m_pResourceManager->GetMinClientRequirement () > "1.3.2-xx" ) // Todo: On merge: Please insert the revision
                {
                    ...
                    CVehicle* pVehicle =
                        CStaticFunctionDefinitions::CreateVehicle(pResource, usModel, vecPosition, vecRotation, strNumberPlate, ucVariant, ucVariant2, bSynced, bServersideOnly);
                    if (pVehicle)
                    {
