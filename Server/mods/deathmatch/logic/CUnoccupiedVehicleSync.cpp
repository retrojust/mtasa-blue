...
void CUnoccupiedVehicleSync::UpdateVehicle(CVehicle* pVehicle)
{
    ...
    if (!pVehicle->IsUnoccupiedSyncable() || pVehicle->IsServersideOnly())
    {
...
void CUnoccupiedVehicleSync::ResyncForPlayer(CPlayer* pPlayer)
{
    ...
    {
        if (vehicle->GetDimension() == pPlayer->GetDimension() && !vehicle->GetFirstOccupant() && vehicle->IsUnoccupiedSyncable() && !vehicle->IsServersideOnly())
        {
